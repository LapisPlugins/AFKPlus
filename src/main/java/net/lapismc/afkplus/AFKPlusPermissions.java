...
public class AFKPlusPermissions extends LapisCorePermissions {
    ...
    private void registerAFKPlusPermissions() {
        for (AFKPlusPermission permission : AFKPlusPermission.values()) {
            registerPermissions(permission.getPermission());
        }
    }

    public enum AFKPlusPermission {

        AFFSelf(new AFKSelf()), AFKOthers(new AFKOthers()), TimeToAFK(new TimeToAFK()),
        TimeToWarning(new TimeToWarning()), TimeToAction(new TimeToAction());

        private final LapisPermission permission;

        AFKPlusPermission(LapisPermission permission) {
